...
class Crawler():
    ...
    def gen_file(feeds):
        ...
        print(f"Creating new file: {f_name}.")
    ...
    def get_page(self, page_num):
        ...
        print("New Items Retrieved:", len(items), "From page:", page_num)

        if not len(items):
            print("Stop reading pages, no more pages to read")
    ...
    def yad2_additions(self, property_id):
        ...
        try:
            ...
            if isinstance(items, dict) and items.get("status_code") == 400:
                ...
                #   'rating_area': []}
                # print(f"status code for id: {property_id}, for item: {items}")
                return 400
        except json.JSONDecodeError as e:
            print(res.text)
            print(res1.text)
            return -1
        # print(f"Retrieved additional info for property {property_id}")
